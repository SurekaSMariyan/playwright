...
const workerFixtures_1 = require("./workerFixtures");
...
exports.test = workerFixtures_1.test.extend({
    ...
    $lang: ({}, use) => use(''),
    $bddContext: async ({ $tags, $test, $step, $bddContextWorker, $lang, $bddTestMeta, $uri, $world }, use, testInfo) => {
        const { config } = $bddContextWorker;
        const bddDataManager = $bddTestMeta && (0, enrichReporterData_1.getEnrichReporterData)(config)
            ? new bddData_1.BddDataManager(testInfo, $bddTestMeta, $uri)
            : undefined;
        await use({
            config,
            testInfo,
    ...
    $test: ({}, use) => use(workerFixtures_1.test),
    $step: ({}, use) => use({ title: '' }),
